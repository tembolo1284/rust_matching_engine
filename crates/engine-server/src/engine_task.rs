...
use engine_core::{InputMessage, MatchingEngine, OutputMessage, MAX_OUTPUTS_PER_ORDER};
use tokio::sync::mpsc;
...
pub async fn run_engine_loop(
    ...
    // Pre-allocate output buffer (reused across all messages) - use ArrayVec for engine API
    ...
    while let Some(request) = engine_rx.recv().await {
        ...
        outputs.clear();
        if let Err(e) = engine.process_message(request.msg, &mut outputs) {
